...
def GenSteps(api):
  ...
  if not api.path.exists(api.path['subordinate_build'].join('v8')):
    ...
    api.zip.unzip('unzipping',
                  api.path['subordinate_build'].join('v8.zip'),
                  api.path['subordinate_build'].join('v8'))
  api.git('checkout', '-f', 'main', cwd=api.path['subordinate_build'].join('v8'))
  api.git('svn', 'rebase', cwd=api.path['subordinate_build'].join('v8'))
