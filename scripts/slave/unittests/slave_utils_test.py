...
class TestGetZipFileNames(unittest.TestCase):
  ...
  def testNormalBuildName(self):
    (base_name, version_suffix) = subordinate_utils.GetZipFileNames({}, 123)
    self._verifyBaseName(base_name)
    self.assertEqual('_123', version_suffix)

  def testNormalBuildNameTryBot(self):
    build_properties = {'mainname': 'main.tryserver.chromium.linux',
                        'buildnumber': 666}
  ...
  def testNormalBuildNameTryBotExtractNoParentBuildNumber(self):
    build_properties = {'mainname': 'main.tryserver.chromium.linux',
                        'buildnumber': 666}
    def dummy():
      subordinate_utils.GetZipFileNames(build_properties, 123, extract=True)
  ...
  def testWebKitName(self):
    (base_name, version_suffix) = subordinate_utils.GetZipFileNames({}, 123, 456)
    self._verifyBaseName(base_name)
...
class TestGSUtil(unittest.TestCase):
  @mock.patch('__main__.subordinate_utils.GSUtilSetup', return_value='/mock/gsutil')
