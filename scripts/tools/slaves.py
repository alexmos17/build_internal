    ...
    if not options.main:
      ...
      for m_p in mains_path:
        if os.path.isfile(os.path.join(m_p, 'twistd.pid')):
          subordinates.extend(
              chromium_utils.RunSubordinatesCfg(os.path.join(m_p, 'subordinates.cfg')))
      ...
      subordinates = subordinates_list.SubordinatesList(os.path.join(main_path, 'subordinates.cfg'))
    def F(os_type):
