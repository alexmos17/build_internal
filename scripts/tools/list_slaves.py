...
def LoadMain(subordinates, path):
  ...
  cur_subordinates = chromium_utils.RunSubordinatesCfg(os.path.join(path, 'subordinates.cfg'))
  for subordinate in cur_subordinates:
    subordinate['mainname'] = cur_main
...
Note: t is replaced with 'tryserver', 'c' with chromium' and
  ...
  # Generate the list of available mains.  We want any with subordinates.
  mains_path = chromium_utils.ListMains(cue='subordinates.cfg')
